...
public class ContainerShip(double maxSpeed, int maxLoad, double maxTotalWeight, string name)
{
    ...
    public static void ReplaceContainer()
    {
        ...
        if (!int.TryParse(Console.ReadLine(), out var shipChoice) || shipChoice < 1 ||
            shipChoice > ShipManagement.ships.Count)
        {
    ...
    public static void TransferContainerBetweenShips()
    {
        ...
        if (targetShip.containers.Count < targetShip.maxLoad &&
            targetShip.currentLoad + containerToTransfer.weight <= targetShip.maxTotalWeight)
        {
            ...
            Console.WriteLine(
                $"Kontener {containerToTransfer.serialNumber} został przeniesiony ze statku {sourceShip.name} na statek {targetShip.name}.");
        }
        else
        {
            Console.WriteLine(
                "Nie można przenieść kontenera: przekroczono limit masy lub liczby kontenerów na statku docelowym.");
        }
    }
    ...
    public static void DisplayShipInfo()
    {
        ...
        Console.WriteLine(
            $"Statek: {ship.name}, Maksymalna prędkość: {ship.maxSpeed} węzłów, Maksymalna liczba kontenerów:" +
        ...
        if (ship.containers.Any())
        {
            ...
            {
                Console.WriteLine(
                    $"Kontener: Typ = {container.GetType().Name}, Serial = {container.serialNumber}, Waga = {container.weight}");
            }
        }
        else
        {
    ...
    {
        return
            $"Container Ship: Max Speed = {maxSpeed} knots, Max Containers = {containers.Count()}, Max Weight = {maxTotalWeight} tons, Currently Loaded Containers = {containers.Count}";
    }
}