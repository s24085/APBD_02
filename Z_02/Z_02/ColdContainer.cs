...
public class ColdContainer : ContainerGeneral, IHazardNotifier
{
    ...
    private void ValidateProductTypeAndTemperature()
    {
        ...
        {
            throw new InvalidOperationException($"Temperatura kontenera {containerTemp}°C jest za niska dla produktu {productType}, optymalna temperatura to {productTypeMap[productType]}°C.");
        }
        if (containerTemp > productTypeMap[productType]+2)
        {
